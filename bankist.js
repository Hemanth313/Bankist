...
const forrmatdate = function(date,loacle){
...
const displayMovements = function (acc,sort=false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    const displayDate = forrmatdate(date,acc.locale);
...
// const user = 'Jonas Schmedtmann hhh';
...
let currentAccount,timer_reset;
btnLogin.addEventListener('click', function(e) {
  ...
  if(currentAccount?.pin === +(inputLoginPin.value)){
      ...
      inputLoginPin.blur();
      if(timer_reset) clearInterval(timer_reset);
      timer_reset = timer();  
...
const timer = function(){
  const tick = function(){
    const min = Math.trunc(timings/60);
    const sec = timings%60;
...
labelBalance.addEventListener('click',function(){
    ...
    // console.log(m);
    const s = document.querySelector('.movements__value').innerHTML;
    console.log(s);
});
